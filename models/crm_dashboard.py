...
class CRMLead(models.Model):
    ...
    def get_country_revenue(self):
        self._cr.execute('''select utm_source.name,count(case when utm_source.id=crm_lead.source_id then 1 else null end) from utm_source,crm_lead where (crm_lead.create_date >= (CURRENT_DATE - INTERVAL '1 day') and crm_lead.create_date<= CURRENT_DATE)
group by utm_source.name''')
    ...
    def get_country_count(self):
        ...
        for each in each_user:
            ...
            self._cr.execute('''SELECT COUNT(crm_lead.id) FROM crm_lead WHERE (create_date >= (CURRENT_DATE - INTERVAL '1 month') and create_date<= CURRENT_DATE) and type='opportunity' and crm_lead.user_id= %s''' % rec[1])
            ...
            self._cr.execute(
                '''select count(crm_lead.id) as won from crm_lead where (create_date >= (CURRENT_DATE - INTERVAL '1 month') and create_date<= CURRENT_DATE) and crm_lead.stage_id=4 and crm_lead.user_id= %s;''' %
                rec[1])
            ...
            self._cr.execute(
                '''select count(crm_lead.id) as lost from crm_lead where (create_date >= (CURRENT_DATE - INTERVAL '1 month') and create_date<= CURRENT_DATE) and crm_lead.active='false' and type = 'opportunity' and crm_lead.user_id=%s;''' %
                rec[1])
            ...
            self._cr.execute(
                '''select  count(crm_lead.id) as Infollow from crm_lead where (create_date >= (CURRENT_DATE - INTERVAL '1 month') and create_date<= CURRENT_DATE) and crm_lead.stage_id=3 and crm_lead.user_id=%s;''' %
                rec[1])
            ...
            self._cr.execute(
                '''select sum(expected_revenue) from crm_lead where (create_date >= (CURRENT_DATE - INTERVAL '1 month') and create_date<= CURRENT_DATE) and crm_lead.user_id= %s''' % rec[1])
